...
def make_files( fpath, fname):
    ...
    wdir2 = '/hysplit-users/allisonr/VOLCANO/Dispersion/'
    duration = 36 #Forward: (+hours) Backward: (-hours)
    duration2 = 18 #For dispersion run only
    ...
    #Identify ash cloud properties - create dispersion files
    #if (alert_type == 'ash'):
    if (alert_type == 'x'):
        ...
        #Number of days for met fields needed - based on hour of trajectories
        if duration2 > 0:
            ndays2 = (duration2 // 24) + 1
            edate2 = sdate + td(days = ndays2)
        #met = fd.findcycles_archive(sdate,edate,met_model,'Forward')
            met = fd.findcycles_forecast(sdate,met_model)
        if duration2 < 0:
            ndays2 = (duration2 // 24)
            edate2 = sdate + td(days = ndays2)
            met = fd.findcycles_archive(sdate,edate2,met_model,'Back')
        #Write CONTROL file
        control2 = hcontrol.HycsControl(fname='CONTROL.disp' + str(pid), working_directory = wdir2)
        #simulation start date
        control2.add_sdate(sdate)
        #set the duration of the simulation (hours)
        control2.run_duration = duration2
        #set location of volcano
        control2.add_location(latlon = (start_lat, start_lon), alt = alt)
        control2.add_location(latlon = (start_lat, start_lon), alt = hgts)
        control2.vertical_motion = vertical
        control2.ztop = topbound
        control2.outdir = wdir2
        control2.outfile = 'cdump.'+str(pid)
        #set particle sizes
        control2.num_sp = 4   #4 total particles (default values will be used)
        control2.species = ['p006', 'p020', 'p060', 'p200']
        # add met files
        for metdir, metfile in zip(met[0],met[1]):
            control2.add_metfile(metdir, metfile)
        control2.write(annotate = False)
        
    return wdir, pid, wdir2
