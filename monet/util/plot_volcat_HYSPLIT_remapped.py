...
#Determining hours for HYSPLIT based on VOLCAT input
hr2 = [int(i) for i in hr]
hr3 = hr2[0] + 1
#Reading in HYSPLIT data
direct='/pub/ECMWF/JPSS/reventador/'
fname1 = 'cdump.004b'
flist = [direct+fname1]
d1=datetime.datetime(2019, 2, 25, hr2[0], 0)
...
plt.xlim(-78, -76.8)
plt.ylim(-0.9,1.1)
plt.title('Volocat & HYSPLIT at hr '+hr[0]+'\n Ash Top Height (km)',fontsize=16,pad=10)
print('saving','/hysplit-users/allisonr/VOLCANO/Images/VOLCATHYSPLIT_AshTopHeight_hr'+hr[0]+'avg_'+fname1+'_remapped.png')
#monet.plots.savefig('Images/VOLCATHYSPLIT_AshTopHeight_hr'+hr[0]+'avg_'+fname1+'.png')
monet.plots.savefig('/hysplit-users/allisonr/VOLCANO/Images/VOLCATHYSPLIT_AshTopHeight_hr'+hr[0]+'avg_'+fname1+'_remapped.png')
plt.show()
